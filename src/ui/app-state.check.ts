...
const otherRecord = { ...record, id: 'other-record' };
let otherEdit = demoReducer({ ...state, records: [otherRecord] }, { type: 'open-detail', recordId: otherRecord.id });
otherEdit = demoReducer(otherEdit, { type: 'edit-record' });
otherEdit = demoReducer(otherEdit, { type: 'open-summary-sheet', kind: 'analysis' });
if (otherEdit.sheet?.kind !== 'analysis') throw new Error('other record writing should be open');
otherEdit = demoReducer(otherEdit, { type: 'sheet-change', change: { kind: 'analysis', working: { ...otherEdit.sheet.working, user_note: 'another record input during pending save' } } });
const completedWhileEditing = acceptSavedRecord(otherEdit, 'operation', record);
assert.equal(completedWhileEditing.route, 'summary', 'late save must not leave another record editing');
assert.equal(completedWhileEditing.selected_record_id, otherRecord.id);
assert.equal(completedWhileEditing.sheet, otherEdit.sheet, 'unapplied text in the other record must survive');
assert.equal(completedWhileEditing.drafts.edits[otherRecord.id], otherEdit.drafts.edits[otherRecord.id]);
assert.equal(completedWhileEditing.drafts.new, null, 'only the completed draft is removed');
const demoCompletedWhileEditing = demoReducer({ ...otherEdit, save_attempt: { ...otherEdit.save_attempt!, state: 'pending' } }, { type: 'save-result', operationId: 'operation', outcome: 'success' });
assert.equal(demoCompletedWhileEditing.sheet, otherEdit.sheet, 'demo save must preserve another open edit too');
...
const lateCaption = acceptCaptionResult(editedWhileGenerating, requestId, draft.input_revision, 'success', '늦게 도착한 AI 글');
...
assert.equal(lateCaption.sheet?.kind === 'analysis' ? lateCaption.sheet.working.user_note : null, '생성 중 고친 글\n\n늦게 도착한 AI 글', 'a completed caption appends to the latest writing without replacing it');
assert.equal(lateCaption.sheet?.kind === 'analysis' ? lateCaption.sheet.caption_status : null, 'success', 'typing during generation must not discard a valid caption');
...
const acceptedCaption = acceptCaptionResult(tagsOnlyEdit, requestId, draft.input_revision, 'success', '새 AI 글');
...
assert.equal(acceptCaptionResult(requested, 'stale-request', draft.input_revision, 'success', caption), requested);
assert.equal(acceptCaptionResult(demoReducer(requested, { type: 'sheet-cancel' }), requestId, draft.input_revision, 'success', caption).sheet, null, 'canceling prevents late results from reopening the sheet');
...
const acceptedAfterClear = acceptCaptionResult(clearedDuringGeneration, requestId, draft.input_revision, 'success', caption);
assert.equal(acceptedAfterClear.sheet?.kind === 'analysis' ? acceptedAfterClear.sheet.working.user_note : null, caption, 'an active request can append into the now-empty editor');
if (acceptedAfterClear.sheet?.kind !== 'analysis') throw new Error('caption editor must stay open');
const clearedAfterCompletion = demoReducer(acceptedAfterClear, { type: 'sheet-change', change: { kind: 'analysis', working: { ...acceptedAfterClear.sheet.working, user_note: '' } } });
const clearedApplied = preserveAdoptedCaption(clearedAfterCompletion, demoReducer(clearedAfterCompletion, { type: 'sheet-apply' }));
...
for (const original of ['', '  직접 쓴 글\n']) {
  ...
  const result = acceptCaptionResult(pending, pending.sheet.caption_request_id, draft.input_revision, 'success', caption);
...
for (const length of [1997, 1998]) {
  ...
  if (pending.sheet?.kind !== 'analysis' || !pending.sheet.caption_request_id) throw new Error('caption request must start');
  const result = acceptCaptionResult(pending, pending.sheet.caption_request_id, draft.input_revision, 'success', '빛');
  if (result.sheet?.kind !== 'analysis') throw new Error('caption editor must stay open');
...
const failedCaption = acceptCaptionResult(requested, requestId, draft.input_revision, 'failure');
...
const stalePhotoCaption = acceptCaptionResult(replacedDuringRequest, requestId, draft.input_revision, 'success', '이전 사진의 AI 글');
...
assert.deepEqual(out.drafts, { new: null, edits: {} });

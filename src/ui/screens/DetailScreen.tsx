...
export function DetailScreen({ locale, images: _images, record, sheet, image_missing, onBack, onOpenRead: _onOpenRead, onOpenActions, onRequestCloseSheet, onEdit, onRequestDelete, onToggleFavorite, onRetryImage, actionsTriggerRef }: DetailScreenProps) {
  ...
  // A recovered cache file can have the same URI; scope failures to the supplied image instance.
  const [failedImage, setFailedImage] = useState<typeof record.stamp | null>(null);
  ...
  const imageFailed = image_missing || !record.stamp.local_uri || failedImage === record.stamp;
  const retryImage = () => {
    setFailedImage(null);

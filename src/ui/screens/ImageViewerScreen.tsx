...
export function ImageViewerScreen({ locale, source, accessibilityLabel, onClose }: ImageViewerScreenProps) {
  ...
  return (
    <Screen title={t.viewer} onBack={onClose} backLabel={t.back} scrollable={false} footer={<Button label={t.close} onPress={onClose} tone="secondary" />} contentStyle={styles.content}>
      <ZoomableImage locale={locale} source={source} accessibilityLabel={accessibilityLabel} style={styles.frame} />
    </Screen>
  );
}
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  frame: { flex: 1, width: '100%', height: '100%', minHeight: 260, backgroundColor: theme.colors.bgPage },
});

...
function errorMessage(locale: OtpScreenProps['locale'], code: OtpScreenProps['error_code']) {
  ...
  if (code === 'otp_expired') return locale === 'ko' ? '코드가 맞지 않거나 만료됐어요. 확인하거나 다시 받아 주세요.' : 'The code is incorrect or expired. Check it or get another one.';
  ...
  return <SafeAreaView edges={['top', 'right', 'bottom', 'left']} style={styles.page}>
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" bounces={false} showsVerticalScrollIndicator={false}>
        <View style={styles.header}><IconButton label={copy.back} onPress={onChangeEmail}><AppIcon name="arrow-left" size={20} color={theme.colors.ink} /></IconButton><SemanticText accessibilityRole="header" style={styles.headerTitle}>{copy.otpTitle}</SemanticText></View>
        <SemanticText accessibilityRole="header" style={styles.heading}>{copy.otpHeading}</SemanticText>
        <SemanticText style={styles.bodyMuted}>{copy.otpLead(email)}</SemanticText>
        <Field label={copy.otpLabel} value={code} onChangeText={(value) => onChangeCode(value.replace(/\D/g, '').slice(0, 6))} keyboardType="number-pad" textContentType="oneTimeCode" autoComplete="one-time-code" maxLength={6} hint={locale === 'ko' ? '코드를 붙여넣을 수 있어요.' : 'You can paste the code here.'} error={errorMessage(locale, error_code)} />
...
const styles = StyleSheet.create({
  flex: { flex: 1 }, page: { flex: 1, backgroundColor: theme.colors.bgPage }, content: { width: '100%', maxWidth: theme.contentMaxWidth, alignSelf: 'center', flexGrow: 1, paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16, gap: 16 }, header: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 12 }, headerTitle: { color: theme.colors.ink, fontSize: 20, lineHeight: 30, fontWeight: '600' }, heading: { color: theme.colors.ink, fontSize: 28, lineHeight: 42, fontWeight: '600' }, bodyMuted: { color: theme.colors.inkSecondary, fontSize: 16, lineHeight: 24 },
});

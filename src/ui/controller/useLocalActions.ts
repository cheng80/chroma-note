...
import { currentDraft, demoReducer, displayLocale, type DemoAction } from '../demo-state';
...
export function useLocalActions(store: ControllerStore, cancelCaption: () => void, refresh: () => Promise<void>) {
  ...
  return useCallback(async (action: DemoAction) => {
    ...
    if (s.save_attempt && !['saved', 'demo_saved'].includes(s.save_attempt.state) && action.type === 'sheet-apply' && s.sheet?.kind !== 'filter' && currentDraft(s)?.draft_id === s.save_attempt.draft_id) {

...
export function RecordArtwork({ fields, source, locale, exportMode = false, onImageLoad, onImageError, onImagePress, imageButtonRef }: Props) {
  const aiNote = fields.ai_field_note_edited ?? fields.ai_field_note;
  const tags = [...fields.semantic_tags, ...fields.mood_tags];
  const image = <StampImage source={source} resizeMode="contain" style={styles.imageFrame} accessibilityLabel={locale === 'ko' ? '기록의 스탬프' : 'Record stamp'} onLoad={onImageLoad} onError={onImageError} />;
  return <View collapsable={false} style={[styles.paper, !exportMode && styles.card]}>
    {onImagePress && !exportMode ? <Pressable ref={imageButtonRef} accessibilityRole="button" accessibilityLabel={locale === 'ko' ? '스탬프 이미지 확대' : 'Enlarge stamp image'} onPress={onImagePress}>{image}</Pressable> : image}
    <View style={styles.writing}>
      ...
      {aiNote && aiNote !== fields.user_note ? <View style={styles.ai}><Text allowFontScaling={!exportMode} style={styles.label}>{locale === 'ko' ? 'AI가 쓴 글' : 'AI writing'}</Text><Text allowFontScaling={!exportMode} style={styles.note}>{aiNote}</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  imageFrame: { width: '100%', aspectRatio: 1, borderWidth: 0, borderRadius: 0, backgroundColor: theme.colors.bgPage },
  writing: { gap: 14 },

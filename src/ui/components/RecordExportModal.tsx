...
export function RecordExportModal({ record, locale, onClose }: { record: DemoRecord; locale: DisplayLocale; onClose: () => void }) {
  const text = copy[locale];
  const modalRef = useModalA11y({ visible: true });
  ...
  const canSave = supported && imageState === 'loaded' && captureSize.kind === 'ready' && !busy && status !== 'saved';
  ...
  return (
    <Modal animationType="none" onRequestClose={busy ? undefined : onClose} presentationStyle="fullScreen">
      <SafeAreaView ref={modalRef} accessibilityViewIsModal={Platform.OS !== 'web'} edges={['top', 'right', 'bottom', 'left']} style={styles.page}>
        <View style={styles.header}>
          ...
          <IconButton label={text.close} onPress={onClose} disabled={busy} icon="x" />
        </View>
...
const styles = StyleSheet.create({
  ...
  header: { minHeight: 72, paddingHorizontal: theme.spacing.xl, paddingVertical: theme.spacing.md, flexDirection: 'row', alignItems: 'center', gap: theme.spacing.md },
  ...
  content: { paddingHorizontal: theme.spacing.xl, paddingBottom: theme.spacing.xl, gap: theme.spacing.md },
  ...
  footer: { paddingHorizontal: theme.spacing.xl, paddingTop: theme.spacing.md, gap: theme.spacing.sm },
});

...
export function ProcessingStep({ label, status, statusLabel }: { label: string; status: ProcessingStepStatus; statusLabel: string }) {
  ...
  return (
    <Reanimated.View accessible accessibilityRole="text" accessibilityLabel={`${label}, ${statusLabel}`} style={[styles.root, animatedStyle]}>
      <View style={styles.indicator}><AppIcon name={icon} size={18} color={color} /></View>

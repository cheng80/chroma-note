...
export default function AppDemo() {
  const { state, send, images, assets, visibleRecords, draft, record, hasMore, refreshing, resendSeconds } = useAppController();
  ...
  const summaryProps = draft ? {
    locale: state.locale, images, draft, sheet: state.sheet, save_attempt: state.save_attempt, blocking_reason: blockingReason,
    ...
    onSave: () => send({ type: 'save' }), onRetrySave: () => send({ type: 'retry-save' }),
  } : null;

  let content: React.ReactNode = null;
  if (state.route === 'email') content = <EmailScreen locale={state.locale} images={images} email={state.email} status={state.auth.status} error_code={state.auth.error_code} onChangeEmail={(email) => send({ type: 'email', value: email })} onSubmit={() => send({ type: 'submit-email' })} />;

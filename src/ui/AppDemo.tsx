...
function AppContent() {
  ...
  const { state, send, images, assets, visibleRecords, draft, displayDrafts, record, hasMore, refreshing, pullRefreshing, resendSeconds, pendingDeletions, canSaveBeforeLogout, sessionRestoreStatus, retrySessionRestore, beginSessionReauthentication } = useAppController();
  ...
  const anotherSavePending = state.save_attempt && !['saved', 'demo_saved'].includes(state.save_attempt.state) && state.save_attempt.draft_id !== draft?.draft_id;
  const blockingReason = anotherSavePending
    ? state.locale === 'ko' ? '다른 기록의 저장 결과를 먼저 확인해 주세요. 편집 중인 내용은 보관했어요.' : 'Resolve the other record’s save first. Your edits are kept.'
    : draft && (!selectedCandidate || !confirmed || !draft.fields.diary_date || draft.stage !== 'summary')
    ? state.locale === 'ko' ? '내용 확인을 마친 뒤 저장할 수 있어요.' : 'Confirm the result before saving.' : null;
  const summaryProps = draft ? {
    discardSaveTriggerRef: dialogTriggerRef, locale: state.locale, images, draft, sheet: state.sheet, save_attempt: state.save_attempt?.draft_id === draft.draft_id ? state.save_attempt : null, blocking_reason: blockingReason,

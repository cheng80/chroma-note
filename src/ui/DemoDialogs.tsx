...
function ReplacePhotoDialog({ ko, send, restoreFocusRef }: { ko: boolean; send: DemoDialogsProps['send']; restoreFocusRef?: DemoDialogsProps['restoreFocusRef'] }) {
  ...
  const close = (replace: boolean) => {
    confirmed.current = replace;
    setVisible(false);
...
export function DemoDialogs({ locale, dialog, scenario, send, canSaveBeforeLogout = false, restoreFocusRef }: DemoDialogsProps) {
  ...
  if (dialog.kind === 'discard-record-edit') return <ConfirmDialog restoreFocusRef={restoreFocusRef} title={ko ? '변경을 버리고 나갈까요?' : 'Discard changes and leave?'} message={ko ? '저장하지 않은 수정 내용은 사라져요. 기존 기록은 그대로 유지돼요.' : 'Unsaved edits will be discarded. The saved record stays unchanged.'} confirmLabel={ko ? '버리고 나가기' : 'Discard and leave'} cancelLabel={ko ? '계속 편집' : 'Keep editing'} destructive onConfirm={() => send({ type: 'discard-record-edit-confirm' })} onCancel={() => send({ type: 'discard-record-edit-cancel' })} />;

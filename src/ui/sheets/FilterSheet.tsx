...
import { ColorSearchPicker } from '../components/ColorSearchPicker';
...
export function FilterSheet({ locale, sheet, records, onChangeSheet, onApply, onCancel, onClose, restoreFocusRef }: { locale: DisplayLocale; sheet: Extract<NonNullable<import('../contract').SheetState>, { kind: 'filter' }>; records: DemoRecord[]; onChangeSheet: (change: SheetChange) => void; onApply: () => void; onCancel: () => void; onClose: () => void; restoreFocusRef?: React.RefObject<unknown | null> }) {
  ...
  const isAll = !sheet.working.start_date && !sheet.working.end_date && !sheet.working.semantic_tag && !sheet.working.favorite_only && !sheet.working.color;
  return <Sheet title={copy.filter} closeLabel={copy.close} restoreFocusRef={restoreFocusRef} onRequestClose={onClose} footer={<><Button label={copy.apply} onPress={onApply} /><Button label={copy.cancel} onPress={onCancel} tone="secondary" /></>}>
    <SemanticText style={styles.label}>{locale === 'ko' ? '색으로 모아보기' : 'Explore by color'}</SemanticText>
    <ColorSearchPicker locale={locale} selected={sheet.working.color} onChange={(color) => update({ ...sheet.working, color })} />
    ...
    <View style={styles.chips}><FilterChip label={copy.clear} selected={isAll} onPress={() => update({ start_date: null, end_date: null, semantic_tag: null, favorite_only: false, color: null })} />{tags.map((tag) => <FilterChip key={tag} label={tag} selected={sheet.working.semantic_tag === tag} onPress={() => update({ ...sheet.working, semantic_tag: sheet.working.semantic_tag === tag ? null : tag })} />)}</View>
...
const styles = StyleSheet.create({ hint: { color: theme.colors.inkSecondary, fontSize: 14, lineHeight: 21 }, label: { color: theme.colors.ink, fontSize: 14, lineHeight: 21, fontWeight: '700' }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 } });

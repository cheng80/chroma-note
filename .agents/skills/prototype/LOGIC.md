...
Record the answer and artifact path using [SKILL.md](SKILL.md). The pure logic can be reused in a separately authorized implementation; no branch or commit is required to retain the demo.

## Anti-patterns

- **Keep checks focused.** Verify the transitions that answer the question without building a production test suite.

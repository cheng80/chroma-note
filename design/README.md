...
| 04 Confirmations | 새 초안 폐기, 기존 기록 편집 나가기, 사진 교체, 재인증 후 계정 영구 삭제 |
...
현재 Pen은 **최상위 페이지 그룹 12개**다. 기존 보드 9개, 재사용 기록 컴포넌트 1개, 화면 59개를 관련 그룹 안으로 이동했다. 기존 항목 69개와 재사용 컴포넌트 16개의 ID·내용·크기는 유지한다. Layers에서 `00`부터 `11`까지 필요한 그룹을 펼치면 기본 화면·오류·시트·크기별 변형을 함께 찾을 수 있다. 캔버스는 같은 순서의 3열로 배치했다. 새 화면은 해당 페이지 그룹 안에 추가하고 화면 내부 레이어는 기존 프레임을 유지한다. [그룹 구조와 보존 검증](previews/page-groups/validation.json).

| 페이지 그룹 | 기존 항목 수 |
|---|---:|
| 00 디자인 기준과 공통 컴포넌트 | 5 |
| 01 시작과 AI 파일 준비 | 9 |
| 02 로그인과 이메일 인증 | 3 |
| 03 Book과 새 사진 초안 | 7 |
| 04 필터와 유사색 검색 | 6 |
...
| 56 | 기존 기록 편집의 컬러 스케치 기반 AI 문구 추가 |
| 00s, 06a–06g | 스플래시, AI 다운로드 안내·진행·일시 정지·검증, 연결 오류·저장 공간 부족·파일 손상 |

폰 기본은 390×844이며 360×800·320×640 변형을 포함한다. 비교와 요약을 두 단계로 나누고, AI 문구·내 메모·태그는 하나의 문구 편집 시트에서 함께 다루며 날짜/장소·대표색은 별도 바텀시트로 연다. 삭제는 중앙 확인 모달이다. 새 사진 시트의 “초안에 적용”, 기존 기록 시트의 “수정 내용 적용”과 서버의 “저장하기/변경 저장”을 구분한다. 새 사진 초안 1개만 계속 보관한다. 기존 기록 편집은 화면 안에서만 유지하며 미저장 상태로 나가면 폐기 여부를 확인한다. Book에는 편집 중인 기록 목록을 두지 않는다. 저장 결과가 불명확한 요청만 별도로 복구한다. AI 생성 중 입력은 허용하고 완료 시 최신 글 끝에 문구를 추가한다. [편집 설계 미리보기와 검증](previews/record-editing/validation.json).

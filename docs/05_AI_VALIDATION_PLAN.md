...
2026-09-11 최신 사용자 지시: **기능 완료를 우선하고 iPad 검증은 나중에 수행한다. iPad 시뮬레이터를 추가로 켜지 않는다.** 아래 iPad mini 6 성능 기준은 후속 실기기 평가용이다. 기존 iPhone 실측·iPad 실측·현재 iPhone Simulator 기능 검증을 섞지 않는다. 현재 실행·검증 범위는 [프로젝트 현황](03_PROJECT_STATUS.md)을 따른다.
...
선화는 2026-09-11 사용자가 `style1` + 원본 RGB 합성으로 제품 채택을 확정했다. 입력 사진과 해상도·강도를 수정 가능한 모듈로 만들며, 아래 검사는 후보 재선정이 아니라 채택 구현의 회귀·안정성 검사다. “채택”, “상업 배포 조건”, “실제 제품 앱 통합”, “VLM 통합”, “Android 실행”, “Chroma Note 품질”은 서로 다른 판정 칸이다. 하나가 통과했다고 다른 칸을 채우지 않는다. 사진 처리 클라우드 우회, 자체 학습/LoRA/distillation로 문제를 해결하지 않는다.
...
2026-09-12 소형 모델 후속 실측: **사진 분위기 분석과 한국어 문구 생성을 함께 유지**하는 조건으로 LFM2.5-VL-450M과 현재 Qwen3-VL 4B를 같은 사진 5장으로 비교했다. 현재 앱 지시문에서는 LFM 태그/문구 형식 0/5·0/5, Qwen 5/5·5/5였다. 짧은 지시문에서도 LFM은 0/5·2/5이며 사진에 없는 사물 생성이 남아 현재 앱 모델을 유지한다. Qwen도 1장에 키릴 문자 혼입이 있어 형식 통과를 품질 보증으로 바꾸지 않는다. [실측·5장 원문](../experiments/model-selection/SMOKE_RESULTS.md#lfm25-vl-450m과-현재-qwen3-vl-4b-5장-비교), [파일·출처 조사](research/2026-09-12-lightweight-mood-models.md). Qwen3.5-0.8B는 아직 실행하지 않았다.
...
| VLM 근거 | 인물 이름/정확 위치/사건 등 중대한 사실 환각 0건; 보이는 객체 태그 precision ≥90% | `QUALITY_FAIL`로 기록한다. 앱에서는 한계 고지 후 사용자가 글·태그를 수정·비우기·AI 생략할 수 있으며, 이 수용은 품질 PASS나 사실성 보증이 아니다. 개인정보·보안·입력·저장 오류는 별도 차단 조건이다 |
...
| 선화 구조 | 고정 50장×3회의 누락 관찰은 보존. 2026-09-12 사용자 결정으로 일부 윤곽·세부 생략은 앱 내 작은 고지와 결과 확인을 전제로 수용. crop·재배치·잘못된 입력 적용은 금지 | 수용한 AI 표현 한계는 기능 완료 차단에서 제외, 입력/변환/합성 결함은 수정 |
...
이번 기준 변경은 사용자가 선화 표현 한계와 AI 글·태그의 사실성 한계를 받아들인 제품 결정이다. 기존 p044/p046 누락과 VLM 사실성 실패 관찰을 삭제하거나 완전 품질 PASS로 바꾸지 않는다. 고지는 앱 화면에서만 표시하고 export PNG에는 포함하지 않으며, 사용자의 수정·비우기·AI 생략으로 앱 운용을 계속한다. VLM의 품질 실패는 기록으로 유지하고, 개인정보·보안·JSON 구조·입력·저장 오류의 차단 조건은 그대로 둔다.
